...
trait ReferencePayCalculator extends PayPeriodGenerator {
  ...
  def calculateVariablePay(priorFurloughPeriod: PayPeriod,
                                     afterFurloughPayPeriod: Seq[PayPeriod], amount: Amount): Seq[RegularPayment] =
    afterFurloughPayPeriod.map(period => calculateReferencePay(priorFurloughPeriod, period, amount))


  private def calculateReferencePay(priorFurloughPeriod: PayPeriod,
                            afterFurloughPayPeriod: PayPeriod, amount: Amount): RegularPayment = {
