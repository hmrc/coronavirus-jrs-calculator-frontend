...
class ClaimPeriodEndFormProvider @Inject() () extends Mappings with SchemeConfiguration {
  ...
  private def validEndDate(claimStart: LocalDate)(implicit messages: Messages): Constraint[LocalDate] =
    Constraint { claimEndDate =>
      (
        ...
        isSameYear(claimStart, claimEndDate)
      ) match {
