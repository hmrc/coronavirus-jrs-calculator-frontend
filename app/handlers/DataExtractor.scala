...
import models.{Amount, BranchingQuestions, CylbPayment, NicCategory, NonFurloughPay, PaymentFrequency, PensionStatus, Period, ReferencePayData, TopUpPayment, UserAnswers}
import pages._
...
trait DataExtractor extends FurloughPeriodExtractor with PeriodHelper {
  ...
  def extractTopUpPayment(userAnswers: UserAnswers): Option[List[LocalDate]] =
    userAnswers.get(TopUpPeriodsPage)
